...
def build_run_diagnostics(run: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "next_diagnostic_steps": [
            ...
            "Open finn/challenger_status.json and finn/agent_brief.json first when the FINN challenger lane ran (forecast --finn or finn pipeline); status records completed/skipped/failed with remediation hints. Then open finn/comparability_receipt.json: comparable=True requires an exact native cutoff-contract match.",
            "Open appendix/challenger_leaderboard.csv, finn/comparison_manifest.json, finn/scoring_manifest.json, and finn/external_model_metrics.csv when FINN bridge artifacts are attached. Directional rows never receive overall_rank.",
